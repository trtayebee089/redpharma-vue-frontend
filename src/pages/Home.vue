<template>
    <div>
        ...
        <section
            class="bg-white relative flex flex-wrap items-center justify-center gap-4 sm:gap-5 md:gap-8 my-10 fade-up transition">
    ...
    `">
        ...
        <section class="mt-10 relative fade-up">
            <div v-for="(item, index) in layoutItems" :key="index" class="mb-12">
                <div v-if="item.type === 'category'">
                    <div class="flex items-center gap-3 mb-5">
                        <!-- Category image -->
                        <img v-if="item.category.image" :src="item.category.image" :alt="item.category.name"
                            class="w-10 h-10 object-cover rounded-sm border border-gray-200 shadow-sm p-1" />

                        <!-- Category title -->
                        <h2 class="text-2xl md:text-3xl font-bold text-gray-800 relative inline-block">
                            {{ item.category.name }}
                            <span class="absolute left-0 bottom-0 w-16 h-1 bg-green-500 rounded-full"></span>
                        </h2>
                    </div>
