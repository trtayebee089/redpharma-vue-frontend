                    ...
                    <div class="px-6 py-4 space-y-4">
                        ...
                        <form @submit.prevent="submitCheckout" class="mt-0 space-y-4">
                            ...
                            <button type="submit" :disabled="checkoutLoading"
                                class="w-full bg-green-500 text-white py-2 rounded hover:border-red-300 hover:bg-gradient-to-b hover:from-red-500 hover:to-red-800 border border-green-600 font-semibold shadow-sm transition-all disabled:cursor-not-allowed disabled:opacity-60">
...
const { t } = useI18n();
const { submitOrder, shippingZones, getShippingZones, getShippingRate, error, loading: checkoutLoading } = useCheckout();
...
const updateQty = (item, newQty) => {
    const stockQty = item.stock_qty ?? item.available_qty ?? item.qty ?? null;
    if (stockQty != null && newQty > stockQty) {
        cartStore.updateQuantity(item.id, stockQty);
        showToast(`Only ${stockQty} item(s) available in stock for ${item.name}`);
        return;
    }
...
const handleManualInput = (item) => {
    ...
    if (stockQty != null && val > stockQty) {
        cartStore.updateQuantity(item.id, stockQty);
        showToast(`Only ${stockQty} item(s) available in stock.`);
    } else {
...
const submitCheckout = async () => {
    ...
    checkoutForm.idempotencyKey = checkoutForm.idempotencyKey || `checkout-${Date.now()}-${Math.random().toString(36).slice(2, 12)}`;
    ...
    try {
        ...
        if (!result.sale?.id || !result.sale?.reference_no) {
            throw new Error("Order was not saved by the server");
        }
