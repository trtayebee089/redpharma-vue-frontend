            ...
            <div class="flex items-end sm:justify-end pb-1 pb-sm-0">
              ...
              >
                ...
                <span class="text-sm text-green-800 font-medium">
                  In Stock: <span class="font-bold">{{ stockQty }} {{ productDetail.unit ?? 'pcs' }}</span>
                </span>
              </div>
            </div>
          </div>
...
const isStockOut = computed(() => {
  ...
  return (
    stockQty.value !== null && stockQty.value <= 0 ||
...
const stockQty = computed(() => {
  const stock = productDetail.value?.available_qty ?? productDetail.value?.stock_qty ?? productDetail.value?.qty;
