<template>
    <section class="bg-gray-50 text-gray-800 space-y-12 px-4 md:px-8 lg:px-16 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-6 rounded-lg shadow">
            ...
            <div class="flex flex-col justify-between space-y-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Brand: <span class="font-medium">{{ product . brand }}</span>
                    </p>
                    <p class="text-sm text-gray-500">Category: <span class="font-medium">{{ product . category }}</span>
                    </p>
                    ...
                    <div class="flex items-center space-x-4 mt-3">
                        ...
                        <p v-if="product.offerPrice < product.regularPrice" class="line-through text-gray-400">
                            ${{ product . regularPrice . toFixed(2) }}</p>
        ...
        <div>
            <h2 class="text-xl font-semibold mb-4">Alternative Medicines {{ alternativeProducts . length }}</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4"
                v-if="alternativeProducts.length > 0">
                <div v-for="product in alternativeProducts" :key="product.id" class="overflow-visible">
...
<script setup>
    ...
    // Alternative products by generic
    const alternativeProducts = computed(() => {
        if (!product.value) return []

        console.log("Current Product Generic Name:", product.value.genericName)

        const filtered = products.value.filter(
            p => p.genericName === product.value.genericName
        )

        console.log("Alternative Products Found:", filtered)
    ...
    const addToCart = () => {
        ...
        cartStore.toggleCart(true) // open cart sidebar if you have that feature
    }
</script>
