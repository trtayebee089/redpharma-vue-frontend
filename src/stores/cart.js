...
export const useCartStore = defineStore('cart', {
  ...
  actions: {
    ...
    addToCart(product) {
      ...
      if (existing) {
        const stockQty = existing.stock_qty ?? product.stock_qty ?? product.available_qty ?? product.qty ?? null;
        ...
        const stockQty = product.stock_qty ?? product.available_qty ?? product.qty ?? null;
        const qty = stockQty !== null ? Math.min(product.quantity || 1, stockQty) : (product.quantity || 1);
        this.items.push({ ...product, stock_qty: stockQty, quantity: qty })
        this.saveCart()
