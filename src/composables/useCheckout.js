    ...
    const submitOrder = async (checkoutForm) => {
        ...
        try {
            ...
            const payload = {
                ...
                idempotency_key: checkoutForm.idempotencyKey || makeIdempotencyKey(),
            ...
            if (!response.data?.success || !savedSale?.id || !savedSale?.reference_no) {
                throw new Error(response.data?.message || 'Order was not saved by the server');
            }
