            ...
            <div class="mt-2 flex items-center justify-between">
                ...
                <div class="ml-2">
                    ...
                    <div v-else
                        ...
                        <button @click="increaseQty" :disabled="isStockOut || (availableStock != null && quantity >= availableStock)"
                            :title="availableStock != null && quantity >= availableStock ? 'Maximum stock reached' : ''"
...
const availableStock = computed(() => {
    const stock = props.product?.available_qty ?? props.product?.stock_qty ?? props.product?.qty;
    return stock == null ? null : Number(stock);
});

const isStockOut = computed(() => {
    return availableStock.value !== null && availableStock.value <= 0 || props.product.price <= 0;
});

const handleAddToCart = () => {
    cartStore.addToCart({ ...props.product, quantity: quantity.value, stock_qty: availableStock.value });
...
const increaseQty = () => {
    if (availableStock.value !== null && quantity.value >= availableStock.value) {
        push.warning(`Only ${availableStock.value} item(s) available in stock.`);
        return;
    }
