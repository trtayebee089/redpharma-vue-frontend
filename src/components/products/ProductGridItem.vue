<template>
    <div
        ...
        <!-- Stock Out Badge -->
        <div v-if="isStockOut"
            class="absolute top-2 right-2 bg-red-600 text-white text-xs px-2 py-1 rounded-full font-semibold z-10" style="top: 0; border-top-left-radius: 0; border-top-right-radius: 0; border-bottom-right-radius: 10px; border-bottom-left-radius: 10px;">
            Stock Out
        </div>

        <!-- Product Image -->
        <router-link :to="`/products/${product.slug}`" class="hover:underline">
            <img :src="product.image?.trim() ? product.image : defaultImagePlaceHolder" :alt="product.name"
        ...
        <div class="p-3 flex flex-col flex-1">
            ...
            <!-- Add to Cart -->
            <button @click="addToCart(product)" :disabled="isStockOut"
                    :class="[
