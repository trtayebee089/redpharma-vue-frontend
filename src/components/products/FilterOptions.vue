<template>
    <div class="p-4 bg-white rounded-lg shadow w-full transform transition-transform duration-300">
        <h2 class="font-semibold mb-3 text-gray-700">{{ $t('filters.filterBy') }}</h2>

        <div class="mb-4">
            <label class="block mb-1 text-gray-600">{{ $t('filters.maxPrice') }}</label>
            <input type="number" v-model.number="maxPrice" placeholder="0"
                class="w-full border border-gray-300 px-3 py-2 rounded" />
        </div>

        <div class="mb-4">
            <label class="block mb-1 text-gray-600">{{ $t('filters.brand') }}</label>
        ...
        <div class="mb-4">
            <h3 class="block mb-1 text-gray-600">{{ $t('filters.availability') }}</h3>
        ...
        <div class="mb-4">
            <h3 class="block mb-1 text-gray-600">{{ $t('filters.rating') }}</h3>
            <div class="flex flex-col space-y-1">
                <label v-for="star in [5, 4, 3, 2, 1]" :key="star" class="flex items-center space-x-2">
                    ...
                    <span>{{ $t('filters.starsAndUp', { count: star }) }}</span>
                </label>
            </div>
        </div>
