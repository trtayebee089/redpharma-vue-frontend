<template>
    <!-- Offcanvas Drawer -->
    <div class="fixed top-0 right-0 h-full bg-white shadow-2xl z-[51] transform transition-transform duration-300 flex flex-col
    w-[90vw] sm:w-[400px] md:w-[450px] lg:w-[480px] xl:w-[400px]"
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <template v-for="item in cartStore.items" :key="item.id">
                <div
                    class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm hover:shadow-md transition gap-4">
    ...
    <div v-if="cartStore.isCartOpen" @click="cartStore.toggleCart(false)"
        class="fixed inset-0 bg-black/60 z-50 transition-opacity"></div>
</template>
